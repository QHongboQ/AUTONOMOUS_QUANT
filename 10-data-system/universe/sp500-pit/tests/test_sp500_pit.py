...
from sp500_pit.validate import ValidationError, compare_qlib_overlap, terminal_state_difference, validate_intervals
...
class PitAdapterTests(unittest.TestCase):
    ...
    def test_BACKWARD_RECONSTRUCTION(self):
        self.assertEqual(active_symbols(self.intervals(), "2024-01-02"), {"AAA", "BRK.B"})

    def test_CHANGE_EFFECTIVE_DATE(self):
        self.assertEqual(active_symbols(self.intervals(), "2024-01-03"), {"BRK.B", "CCC"})
    ...
    def test_SYMBOL_MAPPING_VERSIONED(self):
        self.assertTrue(all(value.evidence == "SP500_PIT_MAPPING_V1" for value in build_symbol_mapping(self.intervals(), "2010-01-01", "2025-01-01")))

    def test_DOT_DASH_SYMBOL_MAPPING(self):
        symbols = {value.logical_security_identity: value.market_data_symbol for value in build_symbol_mapping(self.intervals(), "2010-01-01", "2025-01-01")}
        self.assertEqual(symbols["BRK.B"], "BRK-B")
