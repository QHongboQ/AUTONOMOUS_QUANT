...
Project-owned Certification authority remains non-delegable: temporal-integrity policy, train/validation/OOS partition authority, sealed-OOS access and ledger, purge/embargo policy values, leakage policy, trial-ledger semantics, pre-registration and policy hashes, acceptance/rejection, CertificationDecision, promotion, and Champion/Challenger lifecycle. Every selected upstream below is only a deterministic evidence producer operating on versioned inputs and returning evidence artifacts.

## Existing skfolio ownership — retain, do not duplicate

The installed isolated runtime is skfolio `1.0.6`; a read-only import confirmed `WalkForward` and `CombinatorialPurgedCV` are available. skfolio is the retained temporal-CV algorithm provider: it provides WalkForward, CombinatorialPurgedCV, purge/embargo mechanics, related cross-validation helpers, and portfolio/risk tooling. Certification owns temporal policy and split authority, including partition definitions and purge/embargo policy values. skfolio does not own sealed-OOS authority, leakage policy, trial accounting, acceptance/rejection, or promotion.

No candidate below replaces skfolio’s CV-algorithm and purge/embargo-mechanics role. In particular, MLFinLab/mlfinpy-style PurgedKFold functionality would duplicate the retained skfolio leaf.
...
|---|---|---|---|---|
| [skfolio/skfolio](https://github.com/skfolio/skfolio) | WalkForward, CombinatorialPurgedCV, purging/embargo mechanics, portfolio/risk tools; date-indexed returns and Certification-declared splits/policy values | Existing `1.0.6`; retained isolated Windows runtime | Temporal-CV algorithm provider only; Certification owns temporal policy, partitions, sealed OOS, and promotion. Operates as an evidence producer when input/split/policy hashes are recorded. | **KEEP** — P1/P2 temporal-CV algorithms |
| [bashtage/arch](https://github.com/bashtage/arch) / [bootstrap docs](https://bashtage.github.io/arch/8.0.0/multiple-comparison/multiple-comparison_examples.html) | `SPA` / `RealityCheck`, `StepM`, `MCS`, `IIDBootstrap`, `MovingBlockBootstrap`, `StationaryBootstrap`; benchmark loss vector plus model-loss matrix (`t × k`, lower loss better) | Active public repository; release `8.0.0` (2025-10-21), public activity 2026-08-10. GitHub metadata reports non-SPDX `Other/NOASSERTION`; the checked-in license text is permissive custom BSD-style. Adoption must pin the exact upstream `LICENSE.md` text with the selected version/commit. Modern CPython scientific package; Windows target compatibility must be confirmed against the selected release wheel. | Directly fills multiple-model comparison gap; no material skfolio/Qlib overlap. Objects have configurable bootstrap RNG state, so task code must seed and record state; results are otherwise pure evidence. | **ADD** — `40-certification-system/multiple-testing-control`, separately authorized installation only |
...
The current upstream `LICENSE.md` directly grants rights to use, copy, modify, merge, publish, distribute, sublicense, and sell, subject to copyright/notice, binary-redistribution, non-endorsement, and disclaimer conditions. This audit records `LICENSE_CLASSIFICATION=PERMISSIVE_CUSTOM_BSD_STYLE`. GitHub's non-SPDX metadata is not treated as a legal blocker. Adoption still requires pinning the exact upstream license text with the selected version/commit; this is a reproducibility and notice-preservation requirement, not a new legal conclusion.
...
40-certification-system/
  temporal-integrity       <- OUR CODE: policy, partitions, leakage, purge/embargo values
  temporal-cv-algorithms   <- skfolio (KEEP): WalkForward, CPCV, purge/embargo mechanics
...
|---|---|---|---|---|---|---|---|
| skfolio | temporal-CV algorithms / portfolio risk | retained algorithm provider; Certification owns temporal policy | established, installed | existing project-approved runtime | already present | KEEP | P1/P2 |
| arch | multiple-model bootstrap evidence | complementary | active release/docs/tests | permissive custom BSD-style; metadata non-SPDX | moderate scientific package | ADD; pin exact license text with version/commit | P1 entry / P2 evidence |
...
CERTIFICATION_TEMPORAL_POLICY_OWNER = OUR_CODE
SKFOLIO_ROLE = TEMPORAL_CV_ALGORITHM_PROVIDER
SEALED_OOS_OWNER = OUR_CODE
PROMOTION_OWNER = OUR_CODE
ARCH_LICENSE_METADATA_NON_SPDX = YES
ARCH_LICENSE_TEXT_PERMISSIVE = YES
ARCH_LICENSE_CLASSIFICATION = PERMISSIVE_CUSTOM_BSD_STYLE
ARCH_LICENSE_PIN_REQUIRED = YES
