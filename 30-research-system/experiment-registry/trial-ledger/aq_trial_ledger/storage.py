...
from .auth import AuthenticationError, Authenticator, RejectingAuthenticator
...
_METADATA_RUNTIME_VALUES = {
    ...
    "research_canonicalization_version": RESEARCH_CANONICAL_V1,
    "anchor_canonicalization_version": ANCHOR_CANONICAL_V1,
}
...
class Ledger:
    ...
    def __init__(
        ...
        ledger_id_factory: Any | None = None, authenticator: Authenticator | None = None,
    ):
        self.path = str(path)
        self._clock = clock
        self._id_factory = id_factory
        self._ledger_id_factory = ledger_id_factory or id_factory
        self._authenticator = authenticator or RejectingAuthenticator()
        ...
        if self._has_initialized_schema():
    ...
    def _read_persisted_head(self) -> tuple[int, str]:
        row = self._db.execute(
        ...
        return int(row["ledger_sequence"]), str(row["global_event_hash"])
    ...
    def _event_hash_matches(self, row: sqlite3.Row, previous_hash: str) -> bool:
        payload = parse_json_strict(row["payload_json"])
        envelope = {
            "hash_domain_version": EVENT_HASH_DOMAIN_V1, "ledger_id": self._metadata("ledger_id"),
            "schema_version": SCHEMA_VERSION, "ledger_sequence": row["ledger_sequence"],
            "previous_global_event_hash": previous_hash, "event_type": row["event_type"],
        ...
        return row["previous_global_event_hash"] == previous_hash and row["global_event_hash"] == event_hash(envelope)

    def _event_by_id_with_valid_hash(self, event_id: str) -> sqlite3.Row | None:
        event = self._db.execute("SELECT * FROM trial_events WHERE event_id = ?", (event_id,)).fetchone()
        if event is None:
            return None
        if int(event["ledger_sequence"]) == 1:
            previous = GENESIS_PREVIOUS_HASH
        else:
            prior = self._db.execute(
                "SELECT global_event_hash FROM trial_events WHERE ledger_sequence = ?",
                (int(event["ledger_sequence"]) - 1,),
            ).fetchone()
            if prior is None:
                return None
            previous = str(prior["global_event_hash"])
        return event if self._event_hash_matches(event, previous) else None

    def _verify_metadata_binding(self) -> bool:
        try:
            rows = self._db.execute("SELECT key, value FROM schema_metadata").fetchall()
            metadata = {str(row["key"]): str(row["value"]) for row in rows}
            expected_keys = {"ledger_id", *_METADATA_RUNTIME_VALUES}
            if set(metadata) != expected_keys or not metadata["ledger_id"]:
                return False
            if any(metadata[key] != value for key, value in _METADATA_RUNTIME_VALUES.items()):
                return False
            genesis = self._db.execute(
                "SELECT * FROM trial_events WHERE ledger_sequence = 1"
            ).fetchone()
            if genesis is None or genesis["event_type"] != "GENESIS":
                return False
            payload = parse_json_strict(genesis["payload_json"])
            return (
                payload.get("ledger_id") == metadata["ledger_id"]
                and payload.get("schema_metadata") == metadata
                and self._event_hash_matches(genesis, GENESIS_PREVIOUS_HASH)
            )
        except (LedgerError, sqlite3.DatabaseError, ValueError):
            return False

    def _authenticate(self, auth_context: Any) -> str:
        try:
            actor_id = self._authenticator.authenticate(auth_context)
        except AuthenticationError as error:
            raise LedgerError(str(error)) from error
        if not isinstance(actor_id, str) or not actor_id:
            raise LedgerError("UNKNOWN_OR_INVALID_AUTHENTICATION")
        return actor_id

    def _verify_actor_identity(self, actor_id: str) -> bool:
        row = self._db.execute("SELECT * FROM actor_identities WHERE actor_id = ?", (actor_id,)).fetchone()
        if row is None:
            return False
        event = self._event_by_id_with_valid_hash(str(row["global_event_id"]))
        if event is None or event["event_type"] not in {"GENESIS", "ACTOR_REGISTERED"}:
            return False
        payload = parse_json_strict(event["payload_json"])
        identity = payload.get("owner_identity" if event["event_type"] == "GENESIS" else "actor_identity")
        bindings = (
            "actor_id", "actor_type", "actor_version", "display_name", "credential_binding",
            "authentication_method", "created_at", "metadata_schema_version",
        )
        return (
            isinstance(identity, Mapping) and event["occurred_at"] == row["created_at"]
            and all(identity.get(field) == row[field] for field in bindings)
        )

    def _verify_lifecycle_row(self, table: str, row: sqlite3.Row, bindings: Mapping[str, str]) -> bool:
        event = self._event_by_id_with_valid_hash(str(row["global_event_id"]))
        if event is None or event["event_type"] != row["event_type"] or event["occurred_at"] != row["occurred_at"]:
            return False
        payload = parse_json_strict(event["payload_json"])
        return all(payload.get(payload_key) == row[row_key] for payload_key, row_key in bindings.items())

    def _target_actor_is_active(self, actor_id: str) -> bool:
        if not self._verify_actor_identity(actor_id):
            return False
        row = self._db.execute(
            "SELECT * FROM actor_status_events WHERE actor_id = ? ORDER BY rowid DESC LIMIT 1", (actor_id,)
        ).fetchone()
        bindings = {
            "status_event_id": "status_event_id", "actor_id": "actor_id", "event_type": "event_type",
            "occurred_at": "occurred_at", "actor_id_by": "actor_id_by", "reason": "reason",
        }
        return row is not None and row["event_type"] == "ACTOR_ACTIVATED" and self._verify_lifecycle_row("actor_status_events", row, bindings)

    def _target_capability_is_active(self, actor_id: str, capability: str) -> bool:
        row = self._db.execute(
            "SELECT * FROM capability_events WHERE actor_id = ? AND capability = ? ORDER BY rowid DESC LIMIT 1",
            (actor_id, capability),
        ).fetchone()
        bindings = {
            ...
            "actor_id_by": "actor_id_by", "reason": "reason",
        }
        return row is not None and row["event_type"] == "CAPABILITY_GRANTED" and self._verify_lifecycle_row("capability_events", row, bindings)
        ...
        with self.lock:
            ...
            try:
                self._db.executemany(
                    ...
                    (("ledger_id", ledger_id), *_METADATA_RUNTIME_VALUES.items()),
                )
                ...
                self._append_event(None, None, "GENESIS", owner, {
                    "ledger_id": ledger_id, "schema_metadata": {
                        "ledger_id": ledger_id, **_METADATA_RUNTIME_VALUES,
    ...
    def _next_sequence(self) -> int:
        row = self._db.execute("SELECT MAX(ledger_sequence) AS value FROM trial_events").fetchone()
    ...
    def _actor_is_active(self, actor_id: str) -> bool:
        return self._target_actor_is_active(actor_id)

    def _capability_is_active(self, actor_id: str, capability: str) -> bool:
        return self._target_capability_is_active(actor_id, capability)
    ...
    def register_actor(self, auth_context: Any, actor_id: str, actor_type: str = "RESEARCH_GENERATOR") -> None:
        def operation() -> None:
            by = self._require(auth_context, Capability.ACTOR_ADMIN)
            occurred_at = self._clock()
            status_event_id = str(self._id_factory())
            identity = {
    ...
    def set_actor_status(self, auth_context: Any, actor_id: str, status: str, reason: str = "") -> None:
        if status not in {"ACTOR_ACTIVATED", "ACTOR_SUSPENDED", "ACTOR_RETIRED"}:
            raise LedgerError("invalid actor status")
        def operation() -> None:
            by = self._require(auth_context, Capability.ACTOR_ADMIN)
            current = self._db.execute("SELECT event_type FROM actor_status_events WHERE actor_id = ? ORDER BY rowid DESC LIMIT 1", (actor_id,)).fetchone()
    ...
    def grant_capability(self, auth_context: Any, actor_id: str, capability: Capability, reason: str = "") -> None:
        def operation() -> None:
            by = self._require(auth_context, Capability.CAPABILITY_ADMIN)
            if not self._target_actor_is_active(actor_id):
                raise LedgerError("CAPABILITY_GRANT_TARGET_NOT_ACTIVE")
            self._append_capability(actor_id, capability.value, "CAPABILITY_GRANTED", by, "V1", reason)
        self._run_write(operation)

    def revoke_capability(self, auth_context: Any, actor_id: str, capability: Capability, reason: str = "") -> None:
        def operation() -> None:
            by = self._require(auth_context, Capability.CAPABILITY_ADMIN)
    ...
    def _policy_is_active(self, policy_id: str, version: str) -> bool:
        row = self._db.execute(
            "SELECT * FROM family_policy_events WHERE family_policy_id = ? "
            "AND family_policy_version = ? ORDER BY rowid DESC LIMIT 1", (policy_id, version),
        ).fetchone()
        bindings = {
            "policy_event_id": "policy_event_id", "family_policy_id": "family_policy_id",
            "family_policy_version": "family_policy_version", "event_type": "event_type",
            "occurred_at": "occurred_at", "actor_id": "actor_id", "reason": "reason",
        }
        return row is not None and row["event_type"] == "FAMILY_POLICY_ACTIVATED" and self._verify_lifecycle_row("family_policy_events", row, bindings)

    def register_family_policy(self, auth_context: Any, policy_id: str, version: str, rules: Mapping[str, Any]) -> None:
        def operation() -> None:
            actor = self._require(auth_context, Capability.FAMILY_POLICY_ADMIN)
    ...
    def _change_policy_state(self, auth_context: Any, policy_id: str, version: str, event_type: str) -> None:
        def operation() -> None:
            actor = self._require(auth_context, Capability.FAMILY_POLICY_ADMIN)
            if not self._db.execute("SELECT 1 FROM family_policy_specs WHERE family_policy_id = ? AND family_policy_version = ?", (policy_id, version)).fetchone():
                raise LedgerError("unknown family policy")
            current = self._db.execute("SELECT event_type FROM family_policy_events WHERE family_policy_id = ? AND family_policy_version = ? ORDER BY rowid DESC LIMIT 1", (policy_id, version)).fetchone()
            ...
            self._db.execute("INSERT INTO family_policy_events VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                            (policy_event_id, policy_id, version, event_type, occurred_at, actor, event_type, global_event_id))

        self._run_write(operation)

    def activate_family_policy(self, auth_context: Any, policy_id: str, version: str) -> None:
        self._change_policy_state(auth_context, policy_id, version, "FAMILY_POLICY_ACTIVATED")

    def retire_family_policy(self, auth_context: Any, policy_id: str, version: str) -> None:
        self._change_policy_state(auth_context, policy_id, version, "FAMILY_POLICY_RETIRED")
    ...
    def register(
        self, auth_context: Any, idempotency_key: str, spec: Mapping[str, Any], *,
        ...
        if trial_kind not in ALLOWED_TRIAL_KINDS:
            raise LedgerError("TRIAL_KIND_NOT_ALLOWED")
        def operation() -> str:
            actor = self._require(auth_context, Capability.TRIAL_REGISTER)
            ...
            self._db.execute("INSERT OR IGNORE INTO research_specs VALUES (?, ?, ?)", (spec_hash, canonical, created_sequence))
            ...
            self._db.execute("INSERT INTO registration_idempotency VALUES (?, ?, ?, ?)",
    ...
    def start_execution(
        self, auth_context: Any, trial_id: str, *, execution_kind: str = "NORMAL",
        original_execution_id: str | None = None, replay_identity: Mapping[str, Any] | None = None,
    ) -> str:
        def operation() -> str:
            actor = self._require(auth_context, Capability.EXECUTION_APPEND)
            registration = self._db.execute("SELECT * FROM trial_registrations WHERE trial_id = ?", (trial_id,)).fetchone()
            if registration is None:
                raise LedgerError("unknown trial")
            if execution_kind == "REPRODUCIBILITY_REPLAY":
                original = self._db.execute("SELECT trial_id FROM execution_records WHERE execution_id = ?", (original_execution_id,)).fetchone()
            ...
            self._db.execute("INSERT INTO execution_records VALUES (?, ?, ?, ?, ?, ?, ?)",
    ...
    def _terminal_execution(self, auth_context: Any, execution_id: str, event_type: str, reason: str = "") -> None:
        if event_type not in {"TRIAL_COMPLETED", "TRIAL_FAILED", "TRIAL_CANCELLED"}:
            raise LedgerError("invalid terminal transition")
        def operation() -> None:
            actor = self._require(auth_context, Capability.EXECUTION_APPEND)
            execution = self._db.execute("SELECT trial_id FROM execution_records WHERE execution_id = ?", (execution_id,)).fetchone()
    ...
    def complete_execution(self, auth_context: Any, execution_id: str) -> None:
        self._terminal_execution(auth_context, execution_id, "TRIAL_COMPLETED")

    def fail_execution(self, auth_context: Any, execution_id: str, reason: str) -> None:
        self._terminal_execution(auth_context, execution_id, "TRIAL_FAILED", reason)

    def cancel_execution(self, auth_context: Any, execution_id: str, reason: str) -> None:
        self._terminal_execution(auth_context, execution_id, "TRIAL_CANCELLED", reason)

    def _attach_reference(
        self, auth_context: Any, trial_id: str, execution_id: str, reference_type: str, locator: str,
        content_hash: str | None, table: str, event_type: str, required_capability: Capability,
    ) -> str:
        def operation() -> str:
            actor = self._require(auth_context, required_capability)
            execution = self._db.execute("SELECT trial_id FROM execution_records WHERE execution_id = ?", (execution_id,)).fetchone()
            ...
            created_sequence = self._next_sequence()
            created_at = self._clock()
            self._db.execute(f"INSERT INTO {table} VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    def attach_result(self, auth_context: Any, trial_id: str, execution_id: str, reference_type: str, locator: str, content_hash: str | None = None) -> str:
        return self._attach_reference(auth_context, trial_id, execution_id, reference_type, locator, content_hash,
                                      "result_references", "RESULT_ATTACHED", Capability.RESULT_ATTACH)

    def attach_artifact(self, auth_context: Any, trial_id: str, execution_id: str, reference_type: str, locator: str, content_hash: str | None = None) -> str:
        return self._attach_reference(auth_context, trial_id, execution_id, reference_type, locator, content_hash,
                                      "artifact_references", "ARTIFACT_ATTACHED", Capability.ARTIFACT_ATTACH)

    def record_protocol_violation(self, auth_context: Any, reason: str, external_reference: str | None = None, content_hash: str | None = None) -> str:
        def operation() -> str:
            actor = self._require(auth_context, Capability.PROTOCOL_VIOLATION_RECORD)
            violation_id = str(self._id_factory())
            created_sequence = self._next_sequence()
            occurred_at = self._clock()
            self._db.execute("INSERT INTO protocol_violations VALUES (?, ?, ?, ?, ?, ?, ?)",
    ...
    def _verify_lifecycle_linkage(self) -> bool:
        ...
        for table, identifier, bindings in projections:
            for row in self._db.execute(f"SELECT * FROM {table}"):
                if not self._verify_lifecycle_row(table, row, bindings):
                    return False
        return True
    ...
    def _verify_authoritative_projections(self) -> bool:
        ...
        events = {
            ...
            for row in self._db.execute("SELECT * FROM trial_events")
        }
        for row in self._db.execute("SELECT * FROM actor_identities"):
            ...
            if (identity_payload is None or event["occurred_at"] != row["created_at"]
                ...
                return False
        for row in self._db.execute("SELECT * FROM research_specs"):
            ...
            if event is None or event["event_type"] != "TRIAL_REGISTERED" or payload.get("research_spec_sha256") != row["canonical_research_spec_sha256"]:
                return False
        for row in self._db.execute("SELECT * FROM family_policy_specs"):
            ...
            if not (payload.get("family_policy_id") == row["family_policy_id"]
                ...
                return False
        for row in self._db.execute("SELECT * FROM trial_registrations"):
            event, payload = events.get(row["created_ledger_sequence"], (None, None))
            ...
            if event is None or event["event_type"] != "TRIAL_REGISTERED" or any(payload.get(key) != row[value] for key, value in bindings.items()):
                return False
        for row in self._db.execute("SELECT * FROM registration_idempotency"):
            trial = self._db.execute("SELECT created_ledger_sequence FROM trial_registrations WHERE trial_id = ?", (row["trial_id"],)).fetchone()
            if trial is None:
                return False
    ...
    def _build_verified_snapshot(self, *, as_of_ledger_sequence: int | None = None, anchor: LedgerAnchorManifest | Mapping[str, Any] | None = None) -> TrialHistorySnapshot:
        ...
        last = self._db.execute("SELECT ledger_sequence, global_event_hash FROM trial_events ORDER BY ledger_sequence DESC LIMIT 1").fetchone()
        if last is None:
            raise LedgerError("genesis event missing")
        sequence = int(last["ledger_sequence"]) if as_of_ledger_sequence is None else as_of_ledger_sequence
        boundary = self._db.execute("SELECT global_event_hash FROM trial_events WHERE ledger_sequence = ?", (sequence,)).fetchone()
        if boundary is None:
            raise LedgerError("unknown snapshot boundary")
        events = []
        for row in self._db.execute("SELECT ledger_sequence, trial_id, execution_id, event_type, actor_id, payload_json, global_event_hash FROM trial_events WHERE ledger_sequence <= ? ORDER BY ledger_sequence", (sequence,)):
            events.append({key: (parse_json_strict(row[key]) if key == "payload_json" else row[key]) for key in row.keys()})
        trials = [dict(row) for row in self._db.execute(
        ...
        evidence = {
            ...
            "research_spec_hashes": [row[0] for row in self._db.execute(
    ...
    def create_anchor(self, auth_context: Any) -> LedgerAnchorManifest:
        def operation() -> LedgerAnchorManifest:
            actor = self._require(auth_context, Capability.ANCHOR_CREATE)
            snapshot = self._build_verified_snapshot()
    ...
    def backup_to(self, destination: str | Path) -> None:
        with self.lock:
            if not self.verify_global_chain():
                raise LedgerError("backup blocked by integrity failure")
    ...
    def verify_restored_backup(
        ...
        try:
            snapshot = restored._build_verified_snapshot(anchor=anchor)
